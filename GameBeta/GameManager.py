...
class GameManager:
    def __init__(self):
        ...
        self.boss_grass=BossGrass(WindowSettings.width//2,WindowSettings.height//2)
        self.boss_water=BossWater(WindowSettings.width//2,WindowSettings.height//2)
        self.boss_fire=BossFire(WindowSettings.width//2,WindowSettings.height//2)
        #创建提示板
    ...
    def update_boss(self):
        ...
        #更新镜头
        self.scene.update_camera(self.player)
        self.boss_grass.update(self.get_time(),self.player.rect.x,self.player.rect.y)
        self.boss_grass.update(self.get_time(),self.player.rect.x,self.player.rect.y)
        self.boss_grass.update(self.get_time(),self.player.rect.x,self.player.rect.y)
    #更新子弹状态
    #更新主人公状态
    ...
    def render_boss_grass(self):
        self.scene.render(self.player,self.boss_grass)#渲染场景
        self.render_guide()#渲染提示
    def render_boss_water(self):
        self.scene.render(self.player,self.boss_water)
    def render_boss_fire(self):
        self.scene.render(self.player,self.boss_fire)
    #渲染提示   
